...
void KCF_HC::KCF_Tracker::init(cv::Mat &img, const cv::Rect & bbox)
{
...
void KCF_HC::KCF_Tracker::setTrackerPose(BBox_c &bbox, cv::Mat & img)
{
    init(img, bbox.get_rect());
}

void KCF_HC::KCF_Tracker::updateTrackerPosition(BBox_c &bbox)
{
    if (p_resize_image) {
        BBox_c tmp = bbox;
        tmp.scale(0.5);
...
KCF_HC::BBox_c KCF_HC::KCF_Tracker::getBBox()
{
    BBox_c tmp = p_pose;
...
void KCF_HC::KCF_Tracker::track(cv::Mat &img)
{
    cv::Mat input_gray, input_rgb = img.clone();
    if (img.channels() == 3){
        cv::cvtColor(img, input_gray, CV_BGR2GRAY);
...
std::vector<cv::Mat> KCF_HC::KCF_Tracker::get_features(cv::Mat & input_rgb, cv::Mat & input_gray, int cx, int cy, int size_x, int size_y, double scale)
{
...
cv::Mat KCF_HC::KCF_Tracker::gaussian_shaped_labels(double sigma, int dim1, int dim2)
{
...
cv::Mat KCF_HC::KCF_Tracker::circshift(const cv::Mat &patch, int x_rot, int y_rot)
{
...
ComplexMat KCF_HC::KCF_Tracker::fft2(const cv::Mat &input)
{
...
ComplexMat KCF_HC::KCF_Tracker::fft2(const std::vector<cv::Mat> &input, const cv::Mat &cos_window)
{
...
cv::Mat KCF_HC::KCF_Tracker::ifft2(const ComplexMat &inputf)
{
...
cv::Mat KCF_HC::KCF_Tracker::cosine_window_function(int dim1, int dim2)
{
...
cv::Mat KCF_HC::KCF_Tracker::get_subwindow(const cv::Mat &input, int cx, int cy, int width, int height)
{
    cv::Mat patch;
...
ComplexMat KCF_HC::KCF_Tracker::gaussian_correlation(const ComplexMat &xf, const ComplexMat &yf, double sigma, bool auto_correlation)
{
...
cv::Point2f KCF_HC::KCF_Tracker::sub_pixel_peak(cv::Point & max_loc, cv::Mat & response)
{
...
double KCF_HC::KCF_Tracker::sub_grid_scale(std::vector<double> & responses, int index)
{
    cv::Mat A, fval;
